class Solution {
    ...
    long long rec(vector<int> &nums,int k,int idx,int f){
        ...
        else if(f==1) dp[k][idx][f]=max(dp[k][idx][f],rec(nums,k-1,idx+1,0)-nums[idx]);
        else dp[k][idx][f]=max(dp[k][idx][f],rec(nums,k-1,idx+1,0)+nums[idx]);
        return dp[k][idx][f];
    }
public:
